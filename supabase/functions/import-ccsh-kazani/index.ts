...
import {
  ...
  scrapeSermonPage,
  getTotalPages,
} from "../_shared/ccsh-kazani-scraper.ts";
  ...
  try {
    ...
    if (action === "bulk" || action === "incremental") {
      ...
      if (action === "bulk") {
        ...
        pagesToScrape = Array.from({ length: totalPages }, (_, i) => i * 8);
        log(`Bulk import: ${totalPages} pages to scrape`);
      } else {
      ...
      for (const offset of pagesToScrape) {
        ...
        for (const item of listings) {
          ...
          await delay(1000); // 1s between fetches
    ...
    console.error("import-ccsh-kazani error:", e);
    return new Response(
